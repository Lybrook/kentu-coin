    ...
    <div className="min-h-screen bg-honeydew">
      ...
      <header className="bg-white border-b border-evergreen/10 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center">
          ...
          <button
            ...
            className="p-2 hover:bg-honeydew rounded-lg transition-colors"
          >
            {showBalance ? (
              <Eye className="h-5 w-5 text-evergreen" />
            ) : (
              <EyeOff className="h-5 w-5 text-evergreen" />
            )}
          </button>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-lg border-2 border-evergreen/20 p-6 shadow-sm mb-8">
          ...
          <div className="flex items-center gap-3 bg-honeydew p-4 rounded-lg">
            <code className="flex-1 text-sm font-mono text-evergreen">
              0x1234567890abcdef1234567890abcdef12345678
            </code>
            <Button
              variant="ghost"
              size="sm"
              className="text-evergreen hover:bg-white"
            >
        ...
        <div className="space-y-4">
          {assets.map((asset) => (
            <div
              key={asset.symbol}
              className="bg-white rounded-lg border-2 border-evergreen/20 p-6 shadow-sm"
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-full bg-gold/20 flex items-center justify-center text-xl font-bold text-gold">
                    {asset.icon}
                  </div>
                  <div>
                    <h3 className="font-bold text-evergreen">{asset.symbol}</h3>
                ...
                <div className="text-right">
                  {showBalance ? (
                    <>
                      <p className="font-bold text-evergreen">{asset.balance} {asset.symbol}</p>
                      <p className="text-sm text-gray-600">{asset.value}</p>
                    </>
                  ) : (
                    <p className="font-bold text-evergreen">••••••</p>
                  )}
                </div>
              </div>
              <div className="mt-4 pt-4 border-t border-gray-100 flex justify-between items-center">
                ...
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" className="text-evergreen border-evergreen/30">
